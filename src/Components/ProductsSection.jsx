...
const ProductsSection = () => {
  ...
  const [deleteDialog, setDeleteDialog] = useState({ open: false, productId: null, productName: '', type: 'single' });
  ...
  useEffect(() => {
    applyFilters();
  }, [products, category, subcategory, thirdLevelCategory, fourthLevelCategory, searchTerm]);
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      console.log('Fetching products from:', `${import.meta.env.VITE_BACKEND_URL}/api/v1/product/getAllProducts`);
      
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/product/getAllProducts?page=1&perPage=1000`,
        { 
          withCredentials: true,
  ...
  const applyFilters = () => {
    ...
    if (category && category !== "None") {
      ...
      if (selectedCat) {
        filtered = filtered.filter(product => product.categoryId === selectedCat._id);
      }
    }

    // Filter by subcategory
    if (subcategory && subcategory !== "None") {
      const selectedSubCat = allSubCategories.find(subCat => subCat.name === subcategory);
      if (selectedSubCat) {
        filtered = filtered.filter(product => product.subCatId === selectedSubCat._id);
      }
    }

    // Filter by third level category
    if (thirdLevelCategory && thirdLevelCategory !== "None") {
      const selectedThirdCat = allThirdLevelCategories.find(thirdCat => thirdCat.name === thirdLevelCategory);
      if (selectedThirdCat) {
        filtered = filtered.filter(product => product.thirdSubCatId === selectedThirdCat._id);
      }
    }

    // Filter by fourth level category
    if (fourthLevelCategory && fourthLevelCategory !== "None") {
      const selectedFourthCat = allFourthLevelCategories.find(fourthCat => fourthCat.name === fourthLevelCategory);
      if (selectedFourthCat) {
        filtered = filtered.filter(product => product.fourthSubCatId === selectedFourthCat._id);
      }
    }
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (thirdCategoryRef.current && !thirdCategoryRef.current.contains(event.target)) {
        setIsThirdOpen(false);
      }
      if (fourthCategoryRef.current && !fourthCategoryRef.current.contains(event.target)) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (type === 'single') {
      ...
      try {
        await axios.delete(
          `${import.meta.env.VITE_BACKEND_URL}/api/v1/product/deleteProduct/${productId}`,
          { withCredentials: true }
        );
    ...
    setDeleteDialog({ open: false, productId: null, productName: '', type: 'single' });
  };
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center justify-between px-2 mb-5">
        <p className="text-[18px] font-semibold text-gray-800">Products ({filteredProducts.length})</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4 min-w-0">
            ...
            <div className="relative w-full z-60" ref={thirdCategoryRef}>
              ...
              <div className="relative">
                ...
                />
                <div className="absolute right-3 top-3 text-gray-400 text-lg pointer-events-none">
                  {isThirdOpen ? <RiArrowDropUpFill /> : <RiArrowDropDownFill />}
                </div>
                {isThirdOpen && (
                  <ul className="absolute top-12 left-0 w-full bg-white shadow-lg rounded-md border border-gray-200 z-[80] max-h-48 overflow-y-auto">
                    <li
            ...
            <div className="relative w-full z-60" ref={fourthCategoryRef}>
              <label className="block text-sm font-medium text-gray-700 mb-1">Fourth Level</label>
              <div className="relative">
                <input
                  type="text"
                ...
                />
                <div className="absolute right-3 top-3 text-gray-400 text-lg pointer-events-none">
                  {isFourthOpen ? <RiArrowDropUpFill /> : <RiArrowDropDownFill />}
                </div>
                {isFourthOpen && (
                  <ul className="absolute top-12 left-0 w-full bg-white shadow-lg rounded-md border border-gray-200 z-[80] max-h-48 overflow-y-auto">
                    <li
                    ...
                    {allFourthLevelCategories.map((fourthCat) => (
                      <li
                        key={fourthCat._id}
                        onClick={() => handleFourthCategorySelect(fourthCat.name)}
                        className="px-3 py-2 text-sm hover:bg-blue-50 cursor-pointer"
                      >
                        {fourthCat.name}
                      </li>
                  ...
                  <tr>
                    <td colSpan="11" className="text-center py-8">
                      <div className="flex flex-col items-center justify-center text-gray-500">
                        ...
                        {(category || subcategory || thirdLevelCategory || fourthLevelCategory || searchTerm) && (
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2 md:gap-4 w-full">
                            ...
                            <div className="w-auto">
                              <h3 
                                ...
                                onClick={() => handleProductNameClick(product._id)}
                              >
                        ...
                        <td className="px-2 py-2 md:px-6">
                          ...
                          <ProgressBar value={Math.min((product.stock / 100) * 100, 100)} type="success" />
                        </td>
                        <td className="px-2 py-2 md:px-6">
      ...
      <Dialog
        ...
        onClose={() => setDeleteDialog({ open: false, productId: null, productName: '', type: 'single' })}
      ...
      >
        ...
        <DialogActions>
          <Button 
            onClick={() => setDeleteDialog({ open: false, productId: null, productName: '', type: 'single' })}
            color="primary"
          >
            Cancel
          </Button>
      ...
      <Dialog
        ...
        onClose={() => setImageModal({ open: false, images: [], currentIndex: 0 })}
        maxWidth="lg"
        fullWidth
        PaperProps={{
          style: {
